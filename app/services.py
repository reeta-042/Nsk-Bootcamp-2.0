...
def get_route_from_ors(start_lon: float, start_lat: float, end_lon: float, end_lat: float) -> Dict:
    ...
    try:
        response = httpx.get(ors_url, timeout=30.0) # Increased timeout for robustness
        ...
        raise Exception(f"Failed to get route from OpenRouteService. Raw error: {e}")

# --- Narrative Generation and Reflection Functions ---

def generate_narrative_with_rag(request: models.JourneyRequest, destination_name: str, conversation_history: str = "") -> models.JourneyNarrative:
    """Generates a personalized journey narrative using the RAG model."""
    ...
    prompt = f"""
    You are a Hometown Atlas, a precise and context-aware AI travel guide for African cities and cultures.
    ...
    **CRITICAL INSTRUCTIONS:**
    ...
    5.  **CONVERSATION HISTORY:** Consider the following history of the conversation: {conversation_history}
...
def reflect_and_update_preferences(request: models.ReflectionRequest):
    ...
    - If the user liked a journey about a topic, add that topic to their 'likes'.
    - If they disliked it, add it to their 'dislikes'.
    - Infer topics from the query and title (e.g., 'quiet walk' -> 'quiet', 'historical tour' -> 'history').
    - Do not add duplicate items. Keep the lists concise.
    ...
    **Your Task:** Respond ONLY with the updated JSON object for the 'preferences' field and nothing else.
    ...
    try:
        ...
        st.warning(f"Could not update preferences due to an AI response error: {e}")
